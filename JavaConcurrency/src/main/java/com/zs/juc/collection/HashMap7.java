...
public class HashMap7<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
    ...
    final Entry<K, V> getEntry(Object key) {
        ...
        // 1.获取hashCode
        // 2.hashCode按位与高位参与运算
        // 3 长度取模h & (length - 1)(等价于h%length)
        // 最终：得到bucket的index，通过index获取一个链表，遍历链表获取最终k\v
        int hash = (key == null) ? 0 : hash(key);// hash算法
    ...
    public V put(K key, V value) {
        if (table == EMPTY_TABLE) {
            inflateTable(threshold);// loadfactor=0.75 ;
                                    // capacity=16(table.length); threshold:12
        }
        if (key == null)
            return putForNullKey(value);
        // 1.获取key.hashcode
        // 2.按位与降低hash conflict的可能
        // 3.h & (length - 1)获得buckets数组的索引
        // 4.获取链表 并遍历查找,如果存在对应的key,那么替换覆盖，然后返回旧值
        // 5. 如果这个链表中没有,那么插入

        int hash = hash(key);
        int i = indexFor(hash, table.length);// 获得buckets数组的索引
        for (Entry<K, V> e = table[i]; e != null; e = e.next) {// 遍历这个链表
            Object k;
            if (e.hash == hash && ((k = e.key) == key || key.equals(k))) {// 结点是否已存在这个k\v映射关系
                V oldValue = e.value;
                e.value = value;// 覆盖
                e.recordAccess(this);
                return oldValue;
    ...
    void resize(int newCapacity) {
        ...
        Entry[] newTable = new Entry[newCapacity];// 初始化一个2*当前数组length的新数组
    ...
    void transfer(Entry[] newTable, boolean rehash) {
        ...
        for (Entry<K, V> e : table) {// 遍历这个hash table(桶数组)
            while (null != e) {// 遍历数组中的链表结构
                ...
                // 2,然后仅仅放入单个结点e
                // --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                // 1.newTable[i]!=null表示这个桶已经放入了元素，那么开始形成链表。并且准备把首结点后移，从头部插入一个新结点
                // 2.把首结点后移，从头部插入一个新结点
                e.next = newTable[i];
                newTable[i] = e;
                e = next;// 遍历 继续
    ...
    public Object clone() {
        ...
        if (result.table != EMPTY_TABLE) {
            result.inflateTable(Math.min((int) Math.min(size * Math.min(1 / loadFactor, 4.0f),
                    // we have limits...
                    HashMap7.MAXIMUM_CAPACITY), table.length));
        }
    ...
    void addEntry(int hash, K key, V value, int bucketIndex) {
        //这里与JDK1.6不同，这里先判断—>resize—>再插入新结点
        // 是否需要resize
        if ((size >= threshold) && (null != table[bucketIndex])) {// 当前存在的k\v映射数目>=threshold(默认为16*0.75=12)
            resize(2 * table.length);// 增大一倍
