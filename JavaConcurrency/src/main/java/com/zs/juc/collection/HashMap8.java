...
public class HashMap8<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
     ...
     * upon shrinkage.
     * 当一个桶内的链表数量大于等于8就是用一个tree。
     * 桶链表结构在链表元素大于等于8时转换为tree。
     */
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)//这个bucket数组中对应位置的链表是否存在
            tab[i] = newNode(hash, key, value, null);//table数组对应index未初始化，插入首条
        else {//bucket数组中对应index的链表存在
            Node<K, V> e;
            K k;
            if (p.hash == hash && ((k = p.key) == key || (key != null && key.equals(k))))//判断key\value\key的hash函数返回值 
                e = p;
            else if (p instanceof TreeNode)
                e = ((TreeNode<K, V>) p).putTreeVal(this, tab, hash, key, value);
            else {
                for (int binCount = 0;; ++binCount) {
                    //遍历查找，如果没有重复的，则增加一个结点，链表上结点数达到8个，转换为table为tree
                    if ((e = p.next) == null) {//如果当前链表p只有一个结点
                        p.next = newNode(hash, key, value, null);//构造并增加一个结点
                        if (binCount >= TREEIFY_THRESHOLD - 1) // -1 for 1st (0~7)达到8个结点时
                            treeifyBin(tab, hash);
                        break;
                    }
                    if (e.hash == hash && ((k = e.key) == key || (key != null && key.equals(k))))
            ...
            if (e != null) { // existing mapping for key
                V oldValue = e.value;
                if (!onlyIfAbsent || oldValue == null)//可以覆盖已存在的值？ || oldValue为null
                    e.value = value;//覆盖
                afterNodeAccess(e);
                return oldValue;
            }
        }
        //如果没有找到这个key对应的结点，也就是说没有这个kv映射对
        ++modCount;//插入产生结构改变 +1
        if (++size > threshold)//这里可以看到是先插入一个结点（++size），在考虑扩容。k\v映射数目超过threshold。1.7是==threshold就扩容
            resize();//动态扩容
    ...
    final Node<K, V>[] resize() {
        ...
        int oldCap = (oldTab == null) ? 0 : oldTab.length;//oldCap是原有hashtable的数组length
        int oldThr = threshold;//oldThr是原有threshold
        int newCap, newThr = 0;
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) {//达到max的情况
                threshold = Integer.MAX_VALUE;
                return oldTab;
            } 
            //newCap=原有数组length*2
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY && oldCap >= DEFAULT_INITIAL_CAPACITY)
                newThr = oldThr << 1; // double threshold 扩大两倍容量
        ...
        Node<K, V>[] newTab = (Node<K, V>[]) new Node[newCap];//构造bucket桶数组,使用最新的newCap
        table = newTab;
        if (oldTab != null) {
            for (int j = 0; j < oldCap; ++j) {//将原有数组中的原有链表  各个结点遍历后放入新的扩容数组
                Node<K, V> e;
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null;//for GC
                    if (e.next == null)//只有一个结点,未形成链表
    ...
    final void treeifyBin(Node<K, V>[] tab, int hash) {
        int n, index;
        Node<K, V> e;
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)//如果table数组为null || table数组length<64
            resize();//扩容
        else if ((e = tab[index = (n - 1) & hash]) != null) {//当前key对应的桶不为null
            TreeNode<K, V> hd = null, tl = null;
            do {//构造红黑树
                TreeNode<K, V> p = replacementTreeNode(e, null);//将这个桶中的链表构造为tree
                if (tl == null)
                    hd = p;
                else {
                    p.prev = tl;
                ...
                tl = p;
            } while ((e = e.next) != null);//下个结点存在，也就是没有到链表尾部
    ...
    static class HashMapSpliterator<K, V> {
        ...
        HashMapSpliterator(HashMap8<K, V> m, int origin, int fence, int est, int expectedModCount) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class KeySpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<K> {
        KeySpliterator(HashMap8<K, V> m, int origin, int fence, int est, int expectedModCount) {
    ...
    static final class ValueSpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<V> {
        ValueSpliterator(HashMap8<K, V> m, int origin, int fence, int est, int expectedModCount) {
            super(m, origin, fence, est, expectedModCount);
        }
    ...
    static final class EntrySpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<Map.Entry<K, V>> {
        EntrySpliterator(HashMap8<K, V> m, int origin, int fence, int est, int expectedModCount) {
            super(m, origin, fence, est, expectedModCount);
        }
    ...
    static final class TreeNode<K, V> extends LinkedHashMap.Entry<K, V> {
         ...
         * tree structure).
         */
        final void removeTreeNode(HashMap8<K, V> map, Node<K, V>[] tab, boolean movable) {
            int n;
