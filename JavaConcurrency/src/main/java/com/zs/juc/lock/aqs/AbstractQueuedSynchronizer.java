 ...
 * that rely on a single atomic {@code int} value to represent state. Subclasses
 * must define the protected methods that change this state, and which define
 * what that state means in terms of this object being acquired or released.
 * Given these, the other methods in this class carry out all queuing and
 * blocking mechanics. Subclasses can maintain other state fields, but only the
 * atomically updated {@code int} value manipulated using methods
 * {@link #getState}, {@link #setState} and {@link #compareAndSetState} is
 * tracked with respect to synchronization.
 *
 * 这个类被设计为一个有用的基础类，为大多数的同步器提供基础支持。 这个类主要通过一个int值(private volatile int
 * state)来代表状态。 {@link #getState},
 ...
 * 
 * 这个类的子类应该被定义为非公有的内部帮助类,这个帮助类被用来实现(AQS子类自己内部的)同步属性操作。 这个类没有实现任何同步接口, 它倒是定义了一些
 * {@link #acquireInterruptibly}这样的方法, 主要是为了让实现类自己去实现同步功能。
 *
 * <p>
 ...
 * 然而并不能保证公平或线程无饥饿,早先的排队线程是允许在排在它自己后边的线程之前再次竞争的， 并且每次竞争都会有一个公正的机会去成功的阻止进入的线程。
...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable {
	...
	static final class Node {
		 ...
		 * retry the atomic acquire, and then, on failure, block.
		 * signal：当前节点的后继结点被阻塞或者即将被阻塞(通过 park操作进入waiting状态)，所以当前节点在释放或者被取消时，
		 * 必须unpark它的后继结点。为了避免竞争，acquire方法必须首先保证当前节点需要一个signal通知，然后再次尝试原子性获取，
		 * 然后若失败，则阻塞。
		 * 
		 * <pre/>
		 * CANCELLED: This node is cancelled due to timeout or interrupt. Nodes
		 * never leave this state. In particular, a thread with cancelled node
		 * never again blocks.
		 * 
		 * <pre/>
		 * cancelled:
		 * 当前节点因为超时或者被中断而进入取消状态。取消状态的结点不会被再次阻塞，并且取消状态的结点不会再次改变状态，直到被回收。
		 * 
		 * <pre/>
		 * CONDITION: This node is currently on a condition queue. It will not
		 * be used as a sync queue node until transferred, at which time the
		 * status will be set to 0. (Use of this value here has nothing to do
		 * with the other uses of the field, but simplifies mechanics.)
		 * condition:
		 * 这个结点当前在condition条件队列中。这个结点在被转换(transferred)之前，将不会被同步队列再次使用。
		 * 并且转换时status变量将会被设置为0.
		 * 
		 * <pre/>
		 ...
		 * propagate:一个针对头结点的releaseShared操作将被传播给其他结点。这个状态在doReleaseShared方法中被设置，为了
		 * 保证传播可以继续。即使 其他操作已经进行了干预。
		 * 
		 * 0: None of the above The values are arranged numerically to simplify
		 * use. Non-negative values mean that a node doesn't need to signal. So,
		 * most code doesn't need to check for particular values, just for sign.
		 *
		 ...
		 * 这个引用指向下一个在conditon上等待的结点或者指向Shared结点。
		 * 因为conditon队列结点访问的前提是先要获取排他锁，所以我们需要一个简单的连接队列去维护等待在conditon上的各个结点。
		 * 随后他们被转移到sync队列中去再次获取锁。
	 ...
	 * set. 设置头结点，并且检查后继结点是否是等待的共享模式结点，如果是传播式唤醒连续的共享结点。
	 *
	 * @param node
	 *            the node
	 * @param propagate
	...
	private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
		...
		if (ws > 0) {
			 ...
			 */
			compareAndSetWaitStatus(pred, ws, Node.SIGNAL);// 改变状态，再次尝试获取锁，失败后将会park.
		}
		return false;
	}
	 ...
	 * current thread. 查询是否有其他线程等待时间比当前线程更长
	 * <p>
