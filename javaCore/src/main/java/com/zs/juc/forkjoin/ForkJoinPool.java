...
public class ForkJoinPool extends AbstractExecutorService {
     ...
     * externalPush的完成版本，处理特殊的场景，也是当首个任务的首个子任务提交到池中的第二次初始化。
     * 
     * @param task the task. Caller must ensure non-null.
     */
    ...
    final void externalPush(ForkJoinTask<?> task) {
        ...
        int rs = runState;//一个volatile int变量
        if ((ws = workQueues) != null && (m = (ws.length - 1)) >= 0 &&
     ...
     * 用来创建线程的线程工厂，默认使用defaultForkJoinWorkerThreadFactory
     * 
     * @param handler the handler for internal worker threads that
     * terminate due to unrecoverable errors encountered while executing
     * tasks. For default value, use {@code null}.
     * handler用来处理内部worker线程在执行任务是发生的异常,默认为null
     * 
     ...
     * true表示异步模式,基于FIFO的调度模型。更适合在处理异步事件模式的场景中使用这种模型。
     * 默认使用stack模型LIFO。
     * @throws IllegalArgumentException if parallelism less than or
     *         equal to zero, or greater than implementation limit
    ...
    public ForkJoinPool(int parallelism,
                        ...
                        UncaughtExceptionHandler handler,
                        boolean asyncMode) {
        this(checkParallelism(parallelism),//检查并发数量,不允许小于1而且不允许大于最大worker数量
             checkFactory(factory),//不允许null
             handler,
